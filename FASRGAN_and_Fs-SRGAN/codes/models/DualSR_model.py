...
class DualSRModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            if self.cri_fea:  # load VGG perceptual loss
                ...
                self.cri_fea_mask = train_opt['cri_fea_mask']
    ...
    def optimize_parameters(self, step):
        ...
        self.fake_L, self.fake_Hi, self.mask = self.netG(self.var_L)

        l_g_total = 0
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            if self.cri_pix:  # pixel loss
                l_g_pix =self.cri_pix(self.fake_L, self.var_H)
                ...
                #use the detach
                l_g_pix += self.l_pix_w * self.cri_pix(self.fake_Hi, self.var_H)
                l_g_pix += self.cri_pix(self.mask, self.var_H)
            ...
            if self.cri_fea:
                ...
                if self.cri_fea_high:
                    fake_fea_high = self.netF(self.fake_Hi)
                    l_g_fea_high = self.l_fea_w*self.cri_fea(fake_fea_high, real_fea)
                    l_g_total += l_g_fea_high
                if self.cri_fea_mask:
                    fake_fea_mask = self.netF(self.mask)
                    l_g_fea_mask= self.l_fea_w * self.cri_fea(fake_fea_mask, real_fea)
                    l_g_total += l_g_fea_mask

            else:
                print("no perceptual loss in low and high part")
            # G gan + cls loss
            pred_g_fake = self.netD(self.fake_Hi)
            ...
            pred_g_fake_mask = self.netD(self.mask)
            # pred_d_real_mask = self.netD(self.var_ref).detach()

            l_g_gan_mask= self.l_gan_w * (self.cri_gan(pred_d_real - torch.mean(pred_g_fake_mask), False) +
                                      self.cri_gan(pred_g_fake_mask - torch.mean(pred_d_real), True)) / 2
        ...
        pred_d_fake = self.netD(self.fake_Hi.detach())  # detach to avoid BP to G
        l_d_real = self.cri_gan(pred_d_real - torch.mean(pred_d_fake), True)
        l_d_fake = self.cri_gan(pred_d_fake - torch.mean(pred_d_real), False)

        l_d_total = (l_d_real + l_d_fake) / 2

        pred_d_fake_mask = self.netD(self.mask.detach())
        l_d_real_mask = self.cri_gan(pred_d_real - torch.mean(pred_d_fake_mask), True)
        l_d_fake_mask = self.cri_gan(pred_d_fake_mask - torch.mean(pred_d_real), False)

        l_d_total += (l_d_real_mask + l_d_fake_mask) / 2
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            if self.cri_fea:
                self.log_dict['l_g_fea_mask'] = l_g_fea_mask.item()
            self.log_dict['l_g_gan_mask'] = l_g_gan_mask.item()
        # D
        self.log_dict['l_d_real'] = l_d_real.item()
        self.log_dict['l_d_fake_mask'] = l_d_fake_mask.item()

        if self.opt['train']['gan_type'] == 'wgan-gp':
            self.log_dict['l_d_gp'] = l_d_gp.item()
        # D outputs
        self.log_dict['D_real'] = torch.mean(pred_d_real.detach())
        self.log_dict['D_fake'] = torch.mean(pred_d_fake_mask.detach())
