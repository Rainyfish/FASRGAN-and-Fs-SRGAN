...
class FsSRGANModel(BaseModel):
    def __init__(self, opt):
        super(FsSRGANModel, self).__init__(opt)
        train_opt = opt['train']
    ...
    def print_network(self):
        ...
        s_e, n_e = self.get_network_description(self.netE)
        if isinstance(self.netG, nn.DataParallel):
        ...
        logger.info('Network E structure: {}, with parameters: {:,d}'.format(net_struc_str, n_e))
        logger.info(s_e)
        if self.is_train:
            # Discriminator
