...
class FAFS_SRGANModel(BaseModel):
    def __init__(self, opt):
        super(FAFS_SRGANModel, self).__init__(opt)
        train_opt = opt['train']
        ...
        if self.is_train:
            ...
            if train_opt['atten_weight']>0:
                self.l_atten_w = train_opt['atten_weight']
                self.cri_atten = nn.L1Loss(reduction='none').to(self.device)
            ...
            if train_opt['feature_weight'] > 0:
                ...
                elif l_fea_type == 'Atten':
                    self.cri_fea = nn.L1Loss(reduction='none').to(self.device)
                else:
            ...
            self.l_gan_mask_w = train_opt['gan_mask_weight']
            ...
            #add the parameters of ex mode into the G and D to be optimized. but want to know if the ex part of G and D is the same part
            #do a litte experiment 0303
            ...
            optim_params_G = optim_ex_params
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            pred_g_fake, pred_g_fake_Mask = self.netD(pred_g_fake_feature)

            pred_d_real_feature =self.netE(self.var_ref)
            pred_d_real, pred_d_real_Mask= self.netD(pred_d_real_feature)
            pred_d_real = pred_d_real.detach()
            pred_d_real_Mask = pred_d_real_Mask.detach()


            l_g_gan = self.l_gan_w * (self.cri_gan(pred_d_real - torch.mean(pred_g_fake), False) +
                                      self.cri_gan(pred_g_fake - torch.mean(pred_d_real), True)) / 2

            l_g_gan_mask = self.l_gan_mask_w*(self.cri_gan(pred_d_real_Mask,False)+self.cri_gan(pred_g_fake_Mask,True))/2

            if self.cri_atten:
                l_g_mask =self.l_atten_w*self.cri_atten(self.fake_H,self.var_H)
                l_g_mask2 = l_g_mask.mul(1-torch.sigmoid(pred_g_fake_Mask).detach()).mean()
                l_g_total +=l_g_mask2

            l_g_total+=l_g_gan_mask
        ...
        pred_d_fake, pred_d_fake_Mask = self.netD(pred_d_fake_feature)
        # pred_d_fake = self.netD(self.fake_H.detach())  # detach to avoid BP to G
        l_d_real = self.cri_gan(pred_d_real - torch.mean(pred_d_fake), True)
        l_d_fake = self.cri_gan(pred_d_fake - torch.mean(pred_d_real), False)

        l_d_real_mask = self.cri_gan(pred_d_real_Mask, True)
        l_d_fake_mask = self.cri_gan(pred_d_fake_Mask,False)

        l_d_total_mask = (l_d_real_mask + l_d_fake_mask) / 2

        l_d_total = (l_d_real + l_d_fake) / 2

        l_d_total +=l_d_total_mask
    ...
    def test(self):
        ...
        self.netG.eval()
        with torch.no_grad():
            self.fake_H_feature = self.netE(self.var_L)
            self.fake_H = self.netG(self.fake_H_feature)
            if self.val_lpips:
